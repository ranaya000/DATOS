...
def cargar_datos_sistema():
    ...
    try:
        ...
        # Intentar normalizar campo de Gerencia si existiera en el Excel
        if 'GERENCIA' in df_pecosas.columns:
            df_pecosas['GERENCIA'] = df_pecosas['GERENCIA'].fillna('GENERAL').astype(str)
...
# --- PESTAÑA 4: CONSULTA AVANZADA POR GERENCIA, SUBGERENCIA Y PECOSAS ---
with tab4:
    st.subheader("Consulta por Gerencia / Subgerencia, PECOSA y Pormenores")
    st.markdown("Busca de forma rápida qué solicitó cada unidad organizativa junto con su respectiva información (fechas, productos, PECOSA, cantidades, etc.).")
    
    if not df_pecosas_global.empty:
        # Detectar columnas disponibles para gerencia si existen
        tiene_gerencia = 'GERENCIA' in df_pecosas_global.columns
        
        col_f1, col_f2, col_f3 = st.columns(3)
        
        with col_f1:
            if tiene_gerencia:
                lista_gerencias = ['[TODOS]'] + sorted(df_pecosas_global['GERENCIA'].dropna().unique().tolist())
                f_gerencia = st.selectbox("Gerencia:", lista_gerencias)
            else:
                f_gerencia = '[TODOS]'
                
        with col_f2:
            lista_subg = ['[TODOS]'] + sorted(df_pecosas_global['SUBGERENCIA'].dropna().unique().tolist())
            f_subg = st.selectbox("Subgerencia:", lista_subg)
            
        with col_f3:
            lista_pecosas = ['[TODOS]'] + sorted(df_pecosas_global['PECOSA'].dropna().unique().tolist(), key=lambda x: str(x))
            f_pec = st.selectbox("N° PECOSA:", lista_pecosas)
            
        # Filtro de búsqueda libre para productos o texto adicional
        busqueda_libre = st.text_input("🔍 Buscar por producto, descripción o proveedor específico:")

        # Aplicar filtros
        df_f_p = df_pecosas_global.copy()
        if tiene_gerencia and f_gerencia != '[TODOS]': 
            df_f_p = df_f_p[df_f_p['GERENCIA'] == f_gerencia]
        if f_subg != '[TODOS]': 
            df_f_p = df_f_p[df_f_p['SUBGERENCIA'] == f_subg]
        if f_pec != '[TODOS]': 
            df_f_p = df_f_p[df_f_p['PECOSA'] == str(f_pec)]
            
        if busqueda_libre:
            mask = df_f_p.astype(str).apply(lambda x: x.str.contains(busqueda_libre, case=False, na=False)).any(axis=1)
            df_f_p = df_f_p[mask]
        ...
        m1, m2 = st.columns(2)
        with m1:
            st.metric("Registros Encontrados", len(df_f_p))
        with m2:
            if 'MONTO' in df_f_p.columns or 'Monto Total' in df_f_p.columns:
                col_monto = 'MONTO' if 'MONTO' in df_f_p.columns else 'Monto Total'
                total_monto = pd.to_numeric(df_f_p[col_monto], errors='coerce').sum()
                st.metric("Monto Total Acumulado", f"S/ {total_monto:,.2f}")
            
        st.dataframe(df_f_p, use_container_width=True)
        
        # Botón para descargar el resultado filtrado
        if not df_f_p.empty:
            csv_data = df_f_p.to_csv(index=False).encode('utf-8')
            st.download_button(
                label="📥 Descargar filtrado en CSV",
                data=csv_data,
                file_name="reporte_subgerencia_filtrado.csv",
                mime="text/csv"
            )
    else:
        st.warning("No hay datos de PECOSAS cargados en 'ORDENES DE COMPRA.xlsx'.")
