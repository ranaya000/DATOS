...
# ==============================================================================
# LÓGICA DE NEGOCIO Y CARGA DE DATOS DESDE LOS NUEVOS ARCHIVOS EXCEL
# ==============================================================================
@st.cache_data
def cargar_datos_sistema():
    ...
    # Diccionario para almacenar los dataframes de los nuevos archivos de seguimiento
    dict_seguimiento = {}
    ...
    for nombre_key, filename in archivos_seguimiento.items():
        try:
            ...
            # Cargamos por defecto la primera hoja o una hoja relevante de seguimiento
            ...
            df_temp = pd.read_excel(filename, sheet_name=sheet_to_load)
            # Limpiar nombres de columnas si fuera necesario
...
with tab4:
    st.subheader("Consulta por Gerencia / Subgerencia y Requerimientos (Nuevos Archivos)")
    st.markdown("Seleccione el archivo de seguimiento y busque lo solicitado por cada gerencia o subgerencia junto con sus pormenores.")
    ...
    if dict_seguimiento_global:
        ...
        if not df_actual.empty:
            # Buscador libre para filtrar cualquier texto (gerencia, subgerencia, producto, fecha, pecosa, etc.)
            busqueda_seg = st.text_input("🔍 Buscar gerencia, subgerencia, producto, fecha o detalle en el archivo:")
